...
class Refine(nn.Module):
    ...
    def forward(self, f, corr_feature, pos=None, test=False):
        if test:
            ...
            p0 = (
                F.unfold(f[0], (61, 61), padding=0, stride=4)
                .permute(0, 2, 1)
                .contiguous()
                .view(-1, 64, 61, 61)
            )
            ...
            p1 = (
                F.unfold(f[1], (31, 31), padding=0, stride=2)
                .permute(0, 2, 1)
                .contiguous()
                .view(-1, 256, 31, 31)
            )
            ...
            p2 = (
                F.unfold(f[2], (15, 15), padding=0, stride=1)
                .permute(0, 2, 1)
                .contiguous()
                .view(-1, 512, 15, 15)
            )
...
class Custom(SiamMask):
    ...
    def track_refine(self, pos):
        pred_mask = self.refine_model(
            self.feature, self.corr_feature, pos=pos, test=True
        )
        return pred_mask
