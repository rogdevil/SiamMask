...
parser.add_argument(
    "--config", dest="config", required=True, help="hyper-parameter for SiamMask"
)
parser.add_argument(
    "--resume",
    default="",
...
parser.add_argument(
    "--refine", action="store_true", help="whether use mask refine output"
)
parser.add_argument(
    "--dataset", dest="dataset", default="VOT2018", choices=dataset_zoo, help="datasets"
)
...
parser.add_argument(
    "--save_mask", action="store_true", help="whether use save mask for davis"
)
parser.add_argument(
    "--gt", action="store_true", help="whether use gt rect for davis (Oracle)"
)
...
def siamese_track(
    ...
    if debug:
        ...
        cv2.rectangle(
            im_debug,
            (crop_box_int[0], crop_box_int[1]),
            (crop_box_int[0] + crop_box_int[2], crop_box_int[1] + crop_box_int[3]),
            (255, 0, 0),
            2,
        )
...
def track_vot(
    model, video, hp=None, mask_enable=False, refine_enable=False, device="cpu"
):
    ...
    for f, image_file in enumerate(image_files):
        ...
        if f == start_frame:  # init
            ...
            state = siamese_init(
                im, target_pos, target_sz, model, hp, device
            )  # init tracker
            location = cxy_wh_2_rect(state["target_pos"], state["target_sz"])
            regions.append(1 if "VOT" in args.dataset else gt[f])
        elif f > start_frame:  # tracking
            state = siamese_track(
                state, im, mask_enable, refine_enable, device, args.debug
            )  # track
            if mask_enable:
        ...
        if args.visualization and f >= start_frame:  # visualization (skip lost frame)
            ...
            if gt.shape[0] > f:
                if len(gt[f]) == 8:
                    cv2.polylines(
                        im_show,
                        [np.array(gt[f], np.int).reshape((-1, 1, 2))],
                        True,
                        (0, 255, 0),
                        3,
                    )
            ...
            if len(location) == 8:
                ...
                cv2.polylines(
                    im_show, [location_int.reshape((-1, 1, 2))], True, (0, 255, 255), 3
                )
            else:
            ...
            cv2.putText(
                im_show, str(f), (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2
            )
            cv2.putText(
                im_show,
                str(lost_times),
    ...
    name = (
        args.arch.split(".")[0]
        + "_"
        + ("mask_" if mask_enable else "")
        + ("refine_" if refine_enable else "")
    ...
    logger.info(
        "({:d}) Video: {:12s} Time: {:02.1f}s Speed: {:3.1f}fps Lost: {:d}".format(
    ...
    for k, thr in enumerate(thrs):
        ...
        for j in range(num_object):
            ...
            for i in range(start_frame, end_frame):
                ...
                mask_sum = (pred == 1).astype(np.uint8) + (target_j[i] > 0).astype(
                    np.uint8
                )
...
def track_vos(
    ...
    toc = 0
    pred_masks = (
        np.zeros((object_num, len(image_files), annos[0].shape[0], annos[0].shape[1]))
        - 1
    )
    for obj_id, o_id in enumerate(object_ids):
        ...
        for f, image_file in enumerate(image_files):
            ...
            if f == start_frame:  # init
                ...
                target_sz = np.array([w, h])
                state = siamese_init(
                    im, target_pos, target_sz, model, hp, device=device
                )  # init tracker
            elif end_frame >= f > start_frame:  # tracking
                state = siamese_track(
                    state, im, mask_enable, refine_enable, device=device
                )  # track
                mask = state["mask"]
            toc += cv2.getTickCount() - tic
            ...
            for j, thr in enumerate(thrs):
                logger.info(
                    "Fusion Multi Object{:20s} IOU at {:.2f}: {:.4f}".format(
                        video["name"] + "_" + str(i + 1), thr, multi_mean_iou[i, j]
                    )
                )
    else:
        multi_mean_iou = []
    ...
    if args.save_mask:
        ...
        pred_mask_final = (np.argmax(pred_mask_final, axis=0).astype("uint8") + 1) * (
            np.max(pred_mask_final, axis=0) > state["p"].seg_thr
        ).astype("uint8")
        for i in range(pred_mask_final.shape[0]):
            cv2.imwrite(
                join(video_path, image_files[i].split("/")[-1].split(".")[0] + ".png"),
    ...
    if args.visualization:
        ...
        for f, image_file in enumerate(image_files):
            output = ((0.4 * cv2.imread(image_file)) + (0.6 * mask[f, :, :, :])).astype(
                "uint8"
            )
