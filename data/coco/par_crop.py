...
parser.add_argument(
    "--search_size", type=int, default=511, help="size of cropped search region"
)
...
def crop_img(
    ...
    for track_id, ann in enumerate(anns):
        ...
        cv2.imwrite(
            join(frame_crop_base_path, "{:06d}.{:02d}.x.jpg".format(0, track_id)), x
        )
        ...
        if enable_mask:
            ...
            cv2.imwrite(
                join(frame_crop_base_path, "{:06d}.{:02d}.m.png".format(0, track_id)), x
            )
    ...
    for data_subset in ["val2017", "train2017"]:
        ...
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            ...
            for i, f in enumerate(futures.as_completed(fs)):
                printProgress(
                    i, n_imgs, prefix=data_subset, suffix="Done ", barLength=40
                )
    print("done")
